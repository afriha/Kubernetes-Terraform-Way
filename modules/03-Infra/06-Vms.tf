...
resource "azurerm_virtual_machine" "ControllerVM" {
  ...
  storage_data_disk {
    name = element(
      azurerm_managed_disk.ControllerManagedDisk.*.name,
      count.index,
    )
    ...
    disk_size_gb = element(
      azurerm_managed_disk.ControllerManagedDisk.*.disk_size_gb,
      count.index,
    )
  }
  os_profile {
...
resource "azurerm_virtual_machine" "WorkerVM" {
  ...
  storage_data_disk {
    ...
    lun             = 0
    disk_size_gb = element(
      azurerm_managed_disk.WorkerManagedDisk.*.disk_size_gb,
      count.index,
    )
  }
  os_profile {
